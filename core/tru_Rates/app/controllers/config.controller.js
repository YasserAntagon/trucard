const CONFIG = require('../models/pgConfiguration'),
  ...
  CustKYC = require('../models/customer/custKYCAllModel'),
  CUSTPERMISSION = require('../models/custPermission'),
  ENPERMISSION = require('../models/ENPermission');
module.exports = {
  ...
  show_consumer_configurations: function (req, res) {
    ...
    var parenttruid = req.body.parentTruID ? req.body.parentTruID : req.body.referencetruid;
    var appliedOn = req.body.appliedOn;
    ...
    if (appliedOn === "consumer") {
      delete query['moduleSelf'];
      delete query['moduleSelfReasons'];
      delete query['limitSelf'];
    } else if (appliedOn === "entity") {
      delete query['limit'];
    }
    ...
    function readFromDB() {
      try {
        PERMISSION.aggregate([
          ...
          {
            $project: {
              _id: 0, module: 1, moduleReasons: 1, KYCFlag: 1, appliedOn: 1,
              moduleSelf: { $cond: { if: { $eq: ["$moduleSelf", null] }, then: "$$REMOVE", else: "$moduleSelf" } },
              moduleSelfReasons: { $cond: { if: { $eq: ["$moduleSelfReasons", null] }, then: "$$REMOVE", else: "$moduleSelfReasons" } },
              limitSelf: { $cond: { if: { $eq: ["$limitSelf", null] }, then: "$$REMOVE", else: "$limitSelf" } },
              limit: { $cond: { if: { $eq: ["$limit", null] }, then: "$$REMOVE", else: "$limit" } },
              digitalPayment: 1, bankSlab: 1,
            }
          }
          ...
          if (appliedOn === "consumer") {
            CUSTPERMISSION.aggregate([
              ...
              if (err) {
                ...
                var rfid = req.body.referencetruid;
                if (!rfid) {
                  var custdetails = await CustKYC.findOne({ truID: req.body.truid });
                  rfid = custdetails.refernceTruID;
                }
                if (rfid && (rfid !== "Company" || rfid.substring(0, 4) === "8000")) {
                  ENPERMISSION.aggregate([
                    { $match: { truID: rfid } },
                    {
                      $project: {
                        ...
                        module: returnModule("$module", "status"),
                        moduleReasons: returnModule("$module", "message"),
                        limit: returnlimit("$limitSelf", ["buy", "redeemCash"]),
                      }
                    },
                    ...
                    if (err) {
                      ...
                      if (!aresults.length) {
                        ...
                        if (!results.length) {
                          mergeConfigs(result, aresults)
                        } else {
                          var finalObj = mergeConfigsindv(aresults, results)
  ...
  show_consumer_configurations_all: function (req, res) {
    var kycflag = req.body.kycflag;
    var truid = req.body.truid;
    var parenttruid = req.body.referencetruid;
    var appliedOn = req.body.appliedOn;
...
function returnlimit(obj, typeArr) {
  ...
  typeArr.forEach(ele => {
    ...
    querygenarray7.includes(fieldKey) ? limitobj[`${String(fieldKey)}txnFreeLimit`] = { $toString: `${String(obj)}.${String(type)}.txnFreeLimit` } : undefined;
    querygenarray7.includes(fieldKey) ? limitobj[`${String(fieldKey)}minStockRequired`] = { $toString: `${String(obj)}.${String(type)}.minStockRequired` } : undefined;
