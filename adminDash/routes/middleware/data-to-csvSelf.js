...
var txnOpt = require('../../../model/db/txnOptiomizationMDSelf');
            ...
            await txnOpt.fetchTxnSummary(rTruID, function (result) {
                const updatedArray = result.map((item, index) => {
                    ...
                    return {
                        ...
                        "Rate": item.brate,
                        "Amount": item.amount,
                        "GST_Charged_from_Partner": item.tax,
                ...
                res.setHeader("Content-Disposition", "attachment; filename=selfTxnSummary.csv");
                if (req.body.token) {
                    res.set({
