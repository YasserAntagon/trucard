function openinvoiceSelf(item) {
    let custtid = $(item).attr("data-tosixid");
    let cinv = $(item).attr("data-invoice");
    var isPeople = $(item).attr("data-invFor");
    let trans_type = $(item).attr("data-trastype");
    $status = $(item).attr("data-status");
    $("#myModalPreview").modal("show")
    if (isPeople == "entity") {
        setTimeout(function () {
            if (trans_type == "buy" || trans_type == "redeem" || trans_type == "redeemCash") {
                viewinvoice(cinv, isPeople, trans_type, custtid, $status);
            }
            else if (trans_type == "addMoney" || trans_type == "walletToBank") {
                OnAddMoneyView(cinv, "", trans_type);
            }
...
function OnAddMoneyView(invoice, toSixId, trastype)    // on click on view invoice 
{
    ...
    $.ajax({
        "url": "/eEntity/egetWalletReceipt", "method": "POST", data: json, success: function (res) {
            if (res.status == 200) {
                jsonAddMoney(res.resource)
            }
        }
    })
}
// Add Money Preview
function jsonAddMoney(data) {
    ...
    var UTR = data.UTRNo ? 'UTR No : ' + data.UTRNo + "" : "";
    var acOrigin = data.acOrigin ? 'Origin A/c Name :' + data.acOrigin + "" : "";
    var destinationAC = data.destinationAC ? 'Destination A/c :' + data.destinationAC + "" : "";
    var mode = data.mode == "bankTransfer" ? "Bank Transfer" : data.mode
    var modeStr = 'Mode of pay : ' + mode + "";
    ...
    var ins = {
        ...
        bankTxnID: data.bankTxnID ? "Bank TXN ID : " + data.bankTxnID : data.UTRNumber ? "Bank TXN ID : " + data.UTRNumber : data.UTRNo ? UTR : "",
        paidBy: data.mop ? "Mode of pay : " + data.mop + temppayby : data.mode ? modeStr : "",
        pgType: data.pgType ? data.pgType : data.acOrigin ? acOrigin : "",
        acOrigin: data.acOrigin ? acOrigin : "",
        destinationAC: destinationAC,
        orderdate: fdate,
        bankName: data.bankName ? "Bank : " + data.bankName : data.bankFName ? "Bank : " + data.bankFName : "",
...
var viewinvoice = function (invoiceno, isPeople, type, custtid, status) {
    ...
    $.ajax({
        "url": "/eEntity/getEnPaymentInvoice", "method": "POST", data: json, success: function (a) {
            ...
            let config = a.config;
            if (data.status == 200) {
                const isref = ["refund", "reversal"];
                const ischeck = ["redeemCash"];
                if (a.reqFlag == "entity") {
                    bindEntityinvoice(data.resource, config);
                    $(".target-payment").empty();
                    if ((data.resource.MOP == "others" || data.resource.MOP == "other") && !ischeck.includes(data.resource.type) && !isref.includes(data.resource.status)) {
                        setTimeout(function () {
                            paymentStatus(invoiceno, data.resource.totalAmount, data.resource.type, data.resource.invoice, custtid, status == "REFUND" ? "REFUND" : data.resource.status, data.resource.PGType, isPeople);
                        }, 1000)
                    }
                }
                else {
...
function transferjsonBuilder(a, config) {
    ...
    if (a.status) {
        if (a.status == "success") {
            ...
            transstatus = "Transferred Failed";
        }
    }
    if (a.type == "buyCash") {
        transactiontype = "Buy";
