...
var editable = function () { 
    ...
    $('#txtPartnerCharges').editable({
        ...
        validate: function (value) {
            ...
            $("#txtCompanyShare").html("0")
            var partner = parseFloat($.trim(value));
            var ptcharges = $("#partnerCharges").html();
            var fCTotal = 100 - parseFloat(ptcharges);
            if (parseFloat(partner)) {
                fCTotal = 100 - (parseFloat(ptcharges) + parseFloat(partner));
            }
            if (fCTotal < 0) {
                return 'please enter valid charges';
            }

            else if ((parseFloat(ptcharges) + parseFloat(partner) + fCTotal) == 100) {
                $("#txtPartnerCharges").html($.trim(value))
                $("#txtCompanyShare").html(fCTotal)
                calculateCheck();
            }
            else {
...
function bindTable() {
    ...
    var txnTypeTable = `<tr>
     ...
     <tr>
       <td width="80%">Sell GST %
       <br><small class="text-warning">(applicable on transaction fee + Company / client Loading)</small>
       </td>
       <td width="80%"><a href="#" id="txtSTax" data-type="text" data-pk="1" data-title="Enter GST %"
           class="editable editable-click editable-disabled" data-original-title="" title="">0</a>
         %</td>
         <td class="text text-success">sellgst</td>
     </tr>
     <tr>
