                        ...
                        if (doc.length != 0) {
                            ...
                            for (var i = 0; i < doc.length; i++) {
                                ...
                                if (doc[i].docBackUrl && doc[i].docBackUrl != "") {
                                    ...
                                    var img2 = '<a class="xzoom-container" data-href="' + cdnURL + doc[i].docBackUrl + '" data-src="' + cdnURL + doc[i].docBackUrl + '" onclick="viewKYC(this);"><img class="img" width="100%" height="180px" src="' + cdnURL + doc[i].docBackUrl + '" alt="document"></a>';
                                    xyz += '<div class="box box-widget box-widgets"><div class="box-body"><h5 class="profile-username">' + $box + '</h5></div><div class="box-footer no-padding">' + img2 + '<ul class="nav nav-stacked"></a></li><li><a href="#">Document No <span class="pull-right badge bg-aqua"> <span>' + $docNumber + '</span></span></a></li><li>' + $filename + '</li></ul> </div></div>';
                                }
...
function Deactivate(json, $thisG, pan) // call api
{
    txnCheck(function (status) {
        if (status == true) {
            ...
            $.ajax({
                "url": "/consumerActive/activateAccount", "method": "POST", data: json, success: function (a) {
...
function kycUploadFile(status) {
    ...
    if (!window.File && window.FileReader && window.FileList && window.Blob) {
        ...
        if (proceed) {
            ...
            form_data.append('truID', $cntruid);
            ...
            $.ajax({
                url: "/adminFileupload/uploadConsumerDoc",
                type: "POST",
                data: form_data,
                contentType: false,
                cache: false,
