...
$("#eCompose").change(function () {
    ...
    if (action == "entity" || action=="consumerAll") { 
    ...
    else if (action === "consumer" || action === "consumerAll") {
        $ctruid = $("#txteTruid").val();
        $txtisParent = $("#txtisParent").val();
        ...
        $.ajax({
            "url": "/sendEmail/getAllConsumerSMSEmail", "method": "POST", data: json, success: function (a) {
                ...
                if (a.status == "200") {
                    ...
                    let data = a.body;
                    syncPeople("consumer", data,"Consumer")
                    emailArray = [...new Set(data.map(it => it.email))];
