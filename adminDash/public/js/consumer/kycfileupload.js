...
// $amtruid = "5000883333303676"
$(my_form_id).on("submit", function (event) {
    event.preventDefault();
    $amtruid = $("#txtcTruid").val().trim();
    $cCRNo = $("#txtcrnno").val().trim();
    $(".btnup").show();
    ...
    if (!window.File && window.FileReader && window.FileList && window.Blob) { //if browser doesn't supports File API
        ...
        if (proceed) {
            ...
            form_data.append('CRNNo', $cCRNo);
            form_data.append('flag', "consumer");
            $total = $('.kycdoc').length;
            ...
            let url = $("#login_form").attr("url");

            $('#consumerAddOnLoader').css('display', 'block');
            $.ajax({
                ...
                if (arr.body.status == 200) {
                    $('#consumerAddOnLoader').fadeOut('slow');
                    submitfinalKYC(arr.page);
                }
                else {
                    // alertify.logPosition("top right");
                    alertify.error(arr.body.messege);
                    return false;
                }
            });
        }
