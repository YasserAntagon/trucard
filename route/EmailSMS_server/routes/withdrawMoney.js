...
router.post('/withdrawMoney', (req, res, next) => {
  try {
    ...
    if (array[1] != process.env.token6) {
      ...
      if (resource.mailtype === "both" || resource.mailtype === "email") {
        ...
        let templateFile = "withdrawMoney";
        var options = {
        ...
        request(options, function (error, response, body) {
          let subject = 'Company Withdraw-(' + resource.invoice + ')';
          res.send(body)
      ...
      if (resource.mailtype === "both" || resource.mailtype === "sms") {
        var jsons = {
          "From": "TruCrd",
          "To": resource.mobile,
          "TemplateName": "withdrawMoney",
          "VAR1": "₹ "  + resource.amount,
